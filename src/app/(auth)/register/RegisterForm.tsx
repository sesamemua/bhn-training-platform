...
/** Set when a closed sign-up lets a company-team invitee through. */
...
/**
 * The sign-up form. page.tsx (a server component) decides whether it
 * renders at all: always while sign-up is open, and only for a pending
 * team invite while it is closed.
 */
export function RegisterForm({ invite = null }: { invite?: RegisterInvite | null }) {
  const router = useRouter();
  const [form, setForm] = useState(() => ({
    name: "",
    // An invitee registers with the address the invite went to — the API
    // refuses any other while sign-up is closed, so it is fixed here too.
    email: invite?.email ?? "",
    password: "",
    passwordConfirm: "",
    jobTitle: "",
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl shadow-xl shadow-brand-900/5 border border-line p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {invite && (
              <div role="status" className="bg-brand-50 border border-brand-200 text-fg text-sm rounded-lg px-3 py-2 flex items-start gap-2">
                <Info size={14} className="mt-0.5 shrink-0 text-brand-600" />
                <span>
                  You&apos;re joining <strong>{invite.companyName}</strong>. Create your account with the
            ...
            <div>
              ...
              <input
                ...
                autoComplete="email"
                className={cn(inputCls, invite && "bg-elevated text-muted cursor-not-allowed")}
