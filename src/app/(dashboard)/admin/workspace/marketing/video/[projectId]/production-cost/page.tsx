/**
 * A video project's Production cost tab: vendor quotes, estimates and one
...
export default async function ProductionCostPage({ params }: Props) {
  ...
  return (
    <div className="space-y-6">
      <PageHero
        eyebrow={<><Receipt size={11} /> Video Production · Production cost</>}
        title={project.title}
        description={groups
      ...
      {!groups || !t ? (
        <p className="max-w-3xl rounded-xl border border-dashed border-line px-4 py-8 text-center text-[13px] text-muted">
          No production cost for this project yet. Send the quotes and it will be set up like the BHN Promo Video budget.
        </p>
      ) : (
        // A budget reads down a column: keep label and amount within one eye-span.
