...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  // ── Symposium AV comparison
  {
    ...
    body: "**Three Livecast documents side by side** \u2014 the 2025 quote, the 2025 final invoice, and the 2026 quote \u2014 line by line, with the difference against last year's *actual* on every row. **The invoice is in there for a reason:** 2025 was billed $818.68 over its own estimate, so the 2025 quote column is not what the 2025 Symposium cost. Reading 2026 against the estimate alone would flatter it. **The discount is now part of the comparison.** Every line on all three documents is a *list* amount, and each document then takes a lump off the bottom \u2014 but the lump is not the same size year to year. It deepens from 12.7% to 21.8% to 27.5%. Comparing the printed line amounts therefore says the AV bill is up **41.6%**; what BHN actually pays is up **31.2%**. The table defaults to *after discount* and shows the printed figure underneath each cell, with a *List* toggle for the raw numbers. Spreading a document's discount across its lines is an apportionment rather than what Livecast did line by line \u2014 two of the reductions are plainly item-specific \u2014 so the page says as much: each document's own total is exact, the per-line net figures are indicative. **Every line carries a note** where the documents differ \u2014 the four-speaker PA that was quoted as a two-speaker rig and billed at double, the EPSON laser projectors at $975 each against the NEC's $650, the labour line that ran 24 hours against the 20 quoted, and the switcher and lights that appeared on the invoice and on no estimate. Filter to changed lines only, or to what is new this year. **A separate section covers what changed that is not a price**, which is where the exposure is: BHN is asked to set up the day before (not in the quote), the venue is still listed as \u201cNone\u201d, overtime is now $170/hour per technician past 10 hours with show labour booked at exactly 10, the cancellation window has widened from 48 hours to three weeks, and the deposit terms contradict the payment policy on the face of the same document. Every figure is reconciled against each document's own stated subtotal, discount and total by a test, so a mistyped line fails the build rather than showing a confident wrong number.",
    kind: "feature",
    visibleTo: STAFF,
    daysAgo: 0,
  },
